const fs = require("fs");
const chunk = require("./chunkUtils");

class ChunkParser {
	armorpieces() {
		const armor_base = require("./source_json/armors/armor_base.json");
		const armor_craft_ext = require("./source_json/armors/armor_craft_ext.json");
		const armor_skills_ext = require("./source_json/armors/armor_skills_ext.json");
		...
		for (const [k, v] of armor_base_map) {
			...
			if (skills.skill1_name != 0) {
				let filtered_skill = chunk.advancedSearch(
					skill_levels,
					"base_name_en",
					"level",
					skills.skill1_name,
		...
		fs.writeFile(
			"./database/tools/build_db/armors/armor_pieces.json",
	...
	armors() {
		...
		const armor_pieces = require("./build_db/armors/armor_pieces.json");
		...
		fs.writeFile(
			"./database/tools/build_db/armors/armor_sets.json",
			JSON.stringify(obj, null, 2),
			err => {
				if (err) {
					console.log(err);
	...
	charms() {
		const charm_base = require("./source_json/charms/charm_base.json");
		...
		fs.writeFile(
			"./database/tools/build_db/charms/charms.json",
			JSON.stringify(obj, null, 2),
			err => {
				if (err) {
					console.log(err);
	...
	decorations(noslash) {
		const decoration_base = require("./source_json/decorations/decoration_base.json");
		const skill_levels = require("./source_json/skills/skill_levels.json");
		...
		fs.writeFile(
			"./database/tools/build_db/decorations/decorations.json",
			JSON.stringify(obj, null, 2),
			err => {
				if (err) {
					console.log(err);
	...
	items() {
		const item_base = require("./source_json/items/item_base.json");
		const item_combination_list = require("./source_json/items/item_combination_list.json");
		const location_items = require("./source_json/locations/location_items.json");
		...
		for (const [k, v] of item_base_map) {
			...
			let filtered_locations = chunk.search(location_items, "item", k);
		...
		fs.writeFile(
			"./database/tools/build_db/items/items.json",
			JSON.stringify(obj, null, 2),
			err => {
				if (err) {
					console.log(err);
	...
	monsters() {
		const monster_base = require("./source_json/monsters/monster_base.json");
		const monster_breaks = require("./source_json/monsters/monster_breaks.json");
		const monster_hitzones = require("./source_json/monsters/monster_hitzones.json");
		const monster_ailments = require("./source_json/monsters/monster_ailments.json");
		const monster_habitats = require("./source_json/monsters/monster_habitats.json");
		const monster_rewards = require("./source_json/monsters/monster_rewards.json");
		const monster_weaknesses = require("./source_json/monsters/monster_weaknesses.json");
		...
		for (const [k, v] of monster_base_map) {
			...
			let filtered_ailments = chunk.search(monster_ailments, "base_name_en", k);
			...
			let filtered_breaks = chunk.search(monster_breaks, "base_name_en", k);
			...
			let filtered_habitats = chunk.search(monster_habitats, "base_name_en", k);
			...
			let filtered_hitzones = chunk.search(monster_hitzones, "base_name_en", k);
			...
			let filtered_rewards = chunk.search(monster_rewards, "base_name_en", k);
			...
			let filtered_weaknesses = chunk.search(monster_weaknesses, "name_en", k);
		...
		fs.writeFile(
			"./database/tools/build_db/monsters/monsters.json",
			JSON.stringify(obj, null, 2),
			err => {
				if (err) {
					console.log(err);
	...
	quests() {
		const quest_base = require("./source_json/quests/quest_base.json");
		const quest_monsters = require("./source_json/quests/quest_monsters.json");
		const quest_rewards = require("./source_json/quests/quest_rewards.json");
		...
		for (const [k, v] of quest_base_map) {
			...
			let filtered_monsters = chunk.search(quest_monsters, "base_id", k);
			...
			let filtered_rewards = chunk.search(quest_rewards, "base_id", k);
			if (filtered_rewards.length && Array.isArray(filtered_rewards)) {
				for (const i in filtered_rewards) {
					let obj = {
		...
		fs.writeFile(
			"./database/tools/build_db/quests/quests.json",
			JSON.stringify(obj, null, 2),
			err => {
				if (err) {
					console.log(err);
	...
	skills() {
		const skill_base = require("./source_json/skills/skill_base.json");
		const skill_levels = require("./source_json/skills/skill_levels.json");
		...
		for (const [k, v] of skill_base_map) {
			...
			let filtered_levels = chunk.search(skill_levels, "base_name_en", k);
		...
		fs.writeFile(
			"./database/tools/build_db/skills/skills.json",
			JSON.stringify(obj, null, 2),
			err => {
				if (err) {
					console.log(err);
	...
	kinsects() {
		const kinsect_base = require("./source_json/weapons/kinsect_base.json");
		const kinsect_craft_ext = require("./source_json/weapons/kinsect_craft_ext.json");
		...
		fs.writeFile(
			"./database/tools/build_db/weapons/kinsects.json",
			JSON.stringify(obj, null, 2),
			err => {
				if (err) {
					console.log(err);
	...
	weapons() {
		const weapon_base = require("./source_json/weapons/weapon_base.json");
		const weapon_ammo = require("./source_json/weapons/weapon_ammo.json");
		const weapon_bow_ext = require("./source_json/weapons/weapon_bow_ext.json");
		const weapon_craft = require("./source_json/weapons/weapon_craft.json");
		const weapon_sharpness = require("./source_json/weapons/weapon_sharpness.json");
		...
		fs.writeFile(
			"./database/tools/build_db/weapons/weapons.json",
			JSON.stringify(obj, null, 2),
			err => {
				if (err) {
					console.log(err);
